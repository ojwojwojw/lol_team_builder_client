...
Riot API Key는 백엔드 서버 환경변수로만 관리합니다.  
...
- 관리자 세션, 수동 적재, 저장된 계정 일괄 적재, 응답 결과를 한 화면에서 확인하는 운영 콘솔입니다.
- 왼쪽에서는 Riot ID 기준 수동 적재를 수행하고, 오른쪽에서는 저장된 계정을 선택해 일괄 적재 또는 티어 갱신을 실행합니다.
- Riot API 키는 화면에 노출되지 않으며, 서버 환경변수로 관리됩니다.

### 2. 배치 스케줄러 화면
![Batch Scheduler](docs/images/liot_loader_2.png)

- 저장된 계정을 선택해 작은 배치 단위로 순차 적재하는 배치 운영 화면입니다.
- 최근 경기 수, 배치 크기, 실행 간격을 조정해 장시간 적재 작업을 관리할 수 있습니다.
- 이 화면도 별도 API 키 입력 없이 동작하며, 적재 요청은 서버 환경변수를 사용합니다.

### 3. Firestore 관리 화면
![Firestore Admin](docs/images/liot_loader_3.png)

- 컬렉션 통계, 문서 목록, 문서 상세 JSON, 삭제 도구를 함께 제공하는 Firestore 모니터링 화면입니다.
- 오래된 데이터 정리나 적재 결과 검증 용도로 사용합니다.
- Riot API 키 관리는 이 모니터링 화면과 분리되어 있으며, 서버 환경변수로만 관리합니다.
